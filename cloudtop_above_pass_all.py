...
if ffc==True:
   file_name_list = glob.glob('cloud_pass_frames/{}/*_{}/*'.format(date_to_use,'ffc'))
else:
   file_name_list = glob.glob('cloud_pass_frames/{}/*_{}/*'.format(date_to_use,'rfc'))
...
def extract_timestamp_from_filename(filepath):
    ...
    filename = os.path.splitext(filepath_parts[3])[0]  # remove .png
...
for file_name in file_name_list:
    ...
    if ffc:
        ...
        mask = np.all(cv_grey > WHITENESS_THRESHOLD, axis=-1)
        cv_grey[~mask] = 0
    ...
    if rfc:
        ...
        mask = np.all(img_grey > WHITENESS_THRESHOLD, axis=-1)
        img_grey[~mask] = 0
        cv_grey = cv2.GaussianBlur(img_grey.astype(np.uint8) * 255, (7, 7), 0)
    ...
    if len(pass_number)==2:
        ...
        os.makedirs('figures/{}/pass_{}_{}_{}_{}/'.format(strdate,pass_number[0],pass_number[1],starttimestr,camera), exist_ok=True)
        plt.savefig('figures/{}/pass_{}_{}_{}_{}/'.format(strdate,pass_number[0],pass_number[1],starttimestr,camera)+'{}.png'.format(file_name.split('/')[-1].split('.')[0]))
    else:
        plt.title('Pass {}_{}, Timestamp: {}.'.format(pass_number,camera, timestamp)+'\n Estimated plane pass height between  {}m  and {}m below cloud top'.format(int(pass_diff1),int(pass_diff2)), fontsize=20)
        os.makedirs('figures/{}/pass_{}_{}_{}/'.format(strdate,pass_number,starttimestr,camera), exist_ok=True)
        plt.savefig('figures/{}/pass_{}_{}_{}/'.format(strdate,pass_number,starttimestr,camera)+'{}.png'.format(file_name.split('/')[-1].split('.')[0]))
    plt.cla()
...
df.to_csv('cloud_heights_{}_{}.csv'.format(strdate,camera))
